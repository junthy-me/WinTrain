...
Each exercise entry SHALL reference an `imageAssetName` that maps to an existing `*.imageset` in `Assets.xcassets`. The imageset SHALL contain at least a 1x universal image file.
